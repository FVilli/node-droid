...
export abstract class ENV {
  ...
  static readonly WORKSPACE_FOLDER =
    process.env.WORKSPACE_FOLDER ||
    (process.cwd() === '/app'
      ? '/app/workspace'
      : path.join(process.cwd(), '../workspace'));
  static readonly LLM_API_URL =
    process.env.LLM_API_URL || 'http://localhost:8000/v1';
  ...
  static readonly LLM_TEMPERATURE = parseFloat(
    process.env.LLM_TEMPERATURE || '0.2',
  );
  static readonly LLM_MAX_TOKENS = parseInt(
  ...
  static readonly MAX_TASK_RETRIES = parseInt(
    process.env.MAX_TASK_RETRIES || '3',
    10,
  );
  static readonly MAX_TOOL_CALLS_PER_TASK = parseInt(
    process.env.MAX_TOOL_CALLS_PER_TASK || '30',
    10,
  );
  static readonly STOP_POLICY =
    process.env.STOP_POLICY || 'STOP_ON_FIRST_FAILURE';
  // --- AI / Triggering ---
  static readonly AI_COMMIT_TAG = process.env.AI_COMMIT_TAG || '[ai]';
  static readonly AI_INSTRUCTIONS_FILE =
    process.env.AI_INSTRUCTIONS_FILE || 'AGENTS.md';
  ...
  // --- MQTT Audit ---
  static readonly MQTT_AUDIT_ENABLED =
    process.env.MQTT_AUDIT_ENABLED === 'true';
  static readonly MQTT_AUDIT_URL =
    process.env.MQTT_AUDIT_URL || 'mqtt://localhost:1883';
  ...
  static readonly MQTT_AUDIT_CLIENT_ID =
    process.env.MQTT_AUDIT_CLIENT_ID || `${name}-${process.pid}`;
  static readonly MQTT_AUDIT_TOPIC_PREFIX =
    process.env.MQTT_AUDIT_TOPIC_PREFIX || 'node-droid/audit';
  static readonly MQTT_AUDIT_QOS = parseInt(
    process.env.MQTT_AUDIT_QOS || '0',
    10,
  );
